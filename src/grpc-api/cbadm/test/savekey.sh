...
get() {
	rm -f ${v_PREFIX}.pem
	$APP_ROOT/src/grpc-api/cbadm/cbadm keypair get --config $APP_ROOT/src/grpc-api/cbadm/grpc_conf.yaml -o json --ns ${NM_NAMESPACE} --id ${NM_SSH_KEY} | jq -r ".privateKey" > ${v_CLUSTER_NAME}.pem
